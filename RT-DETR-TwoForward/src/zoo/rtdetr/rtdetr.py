...
class RTDETR(nn.Module):
    ...
    def forward(self, x, targets=None, wNeck=None):
        ...
        # 2024.07.30 @hslee : add backbone_outs, neck_outs to x(dict_keys)
        if wNeck : 
            x.update({'neck_outs': x})
        else :
            x.update({'backbone_outs': x})
