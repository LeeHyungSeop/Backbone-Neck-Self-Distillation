...
def box2gaussian(boxes1, boxes2):
    ...
    # boxes1.shape = [batch_size, #boxes, 4]
    x1, y1, w1, h1 = boxes1[:, 0], boxes1[:, 1], boxes1[:, 2], boxes1[:, 3]
    x2, y2, w2, h2 = boxes2[:, 0], boxes2[:, 1], boxes2[:, 2], boxes2[:, 3]
    
    theta1 = tf.constant(0, dtype=tf.float32)
    theta2 = tf.constant(0, dtype=tf.float32)
...
def kullback_leibler_divergence(mu1, mu2, mu1_T, mu2_T, sigma1, sigma2):
    ...
    item3 = tf.math.log(tf.linalg.det(sigma2_square) / tf.linalg.det(sigma1_square))
